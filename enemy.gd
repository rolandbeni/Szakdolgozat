...
@onready var enemy_scene =$Sprite2D # Load the enemy scene
...
func spawn_enemies(num):
	for i in range(num):
		...
		get_parent().add_child(new_enemy) # Add the enemy to the parent of the character (assuming character is a chid of another node)
'''
